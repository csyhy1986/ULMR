...
class Pair_set(Dataset):
    ...
    def __getitem__(self, index):
        F = torch.from_numpy(self.data[index]['F'])
        F = F / (torch.norm(F) + 1e-9)
        ptsL = torch.from_numpy(self.data[index]['PTLs'])
        ptsR = torch.from_numpy(self.data[index]['PTRs'])
        # F, mask = cv2.findFundamentalMat(ptsL,ptsR,cv2.FM_RANSAC,0.008,0.999)
        # F = torch.from_numpy(F).reshape(-1)
        lbls = torch.from_numpy(self.data[index]['LBLs'])
